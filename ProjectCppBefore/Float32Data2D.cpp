...
CFloat32Data2D::CFloat32Data2D(int _iWidth, int _iHeight, const float* _pfData)
{
	m_bInitialized = false;
...
//----------------------------------------------------------------------------------------
// Create an instance of the CFloat32Data2D class with initialization of the data block. 
CFloat32Data2D::CFloat32Data2D(int _iWidth, int _iHeight, float _fScalar)
{
	m_bInitialized = false;
...
CFloat32Data2D& CFloat32Data2D::operator=(const CFloat32Data2D& _dataIn)
{
	...
	if (m_bInitialized) {
		if (m_iWidth == _dataIn.m_iWidth && m_iHeight == _dataIn.m_iHeight) {
			...
			memcpy(m_pfData, _dataIn.m_pfData, m_iSize * sizeof(float));
		}
		else {
			if (m_pCustomMemory) {
...
bool CFloat32Data2D::_initialize(int _iWidth, int _iHeight, const float* _pfData)
{
	// basic checks
	ASTRA_ASSERT(_iWidth > 0);
	ASTRA_ASSERT(_iHeight > 0);
...
bool CFloat32Data2D::_initialize(int _iWidth, int _iHeight, float _fScalar)
{
	// basic checks
	ASTRA_ASSERT(_iWidth > 0);
	ASTRA_ASSERT(_iHeight > 0);
...
void CFloat32Data2D::_allocateData()
{
	...
	m_ppfData2D = new float * [m_iHeight];
...
void CFloat32Data2D::copyData(const float* _pfData)
{
	// basic checks
	ASTRA_ASSERT(m_bInitialized);
	ASTRA_ASSERT(_pfData != NULL);
...
void CFloat32Data2D::setData(float _fScalar)
{
	// basic checks
	ASTRA_ASSERT(m_bInitialized);
	ASTRA_ASSERT(m_pfData != NULL);
...
CFloat32Data2D& CFloat32Data2D::clampMin(float& _fMin)
{
	ASTRA_ASSERT(m_bInitialized);
	for (size_t i = 0; i < m_iSize; i++) {
		if (m_pfData[i] < _fMin)
...
CFloat32Data2D& CFloat32Data2D::clampMax(float& _fMax)
{
	ASTRA_ASSERT(m_bInitialized);
	for (size_t i = 0; i < m_iSize; i++) {
		if (m_pfData[i] > _fMax)
...
CFloat32Data2D& CFloat32Data2D::operator*=(const float& f)
{
	ASTRA_ASSERT(m_bInitialized);
	for (size_t i = 0; i < m_iSize; i++) {
		m_pfData[i] *= f;
